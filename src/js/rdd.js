...
// --- Version Fetching Logic (Added) ---
async function autoFillVersion() {
    const binType = downloadForm.binaryType.value;
    const chan = downloadForm.channel.value || "LIVE";
    const verInput = downloadForm.version;

    // Visual feedback
    const originalPlaceholder = verInput.placeholder;
    verInput.value = "";
    verInput.placeholder = "Fetching version...";
    
    const targetUrl = `https://clientsettings.roblox.com/v2/client-version/${binType}/channel/${chan}`;
    const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(targetUrl)}`;

    try {
        log(`[+] Fetching latest version for ${binType} @ ${chan}...`);
        ...
        // data.clientVersionUpload is the one usually needed (e.g. "version-xxxxxxxx")
        if (data.clientVersionUpload) {
            verInput.value = data.clientVersionUpload;
            log(`[+] Found version: ${data.clientVersionUpload}`);
            log(`    Bootstrapper Version: ${data.bootstrapperVersion}`);
        } else {
            log(`[!] API returned data, but 'clientVersionUpload' was missing. Raw version: ${data.version}`);
            verInput.value = data.version; // Fallback
        }

    } catch (error) {
        log(`[!] Failed to fetch version info: ${error}`);
        verInput.placeholder = "Error fetching version";
    } finally {
        setTimeout(() => {
            if(!verInput.value) verInput.placeholder = originalPlaceholder;
        }, 2000);
    }
}
// --------------------------------------
...
function main() {
    ...
    if (version) {
        ...
        log(`[INFO] Select a version. You can manually enter a hash, or click the button to fetch the latest one.`);
